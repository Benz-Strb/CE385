...
// Middleware สำหรับตรวจสอบข้อมูลนักเรียน
const validateStudentData = (req, res, next) => {
    const { name, age } = req.body;
    
    // ตรวจสอบว่า name และ age ถูกส่งมาหรือไม่
    if (!name || !age) {
        return res.status(400).json({ error: 'Invalid data' });
    }
    
    // ตรวจสอบว่า name และ age ไม่เป็นค่าว่าง
    if (name.trim() === '' || age === '') {
        return res.status(400).json({ error: 'Invalid data' });
    }

    // ถ้าผ่านการตรวจสอบแล้ว ให้ไปต่อ
    next();
};
...
app.post('/api/students', validateStudentData, (req, res) => {
    const {name, age} = req.body;
    const maxId = Math.max(...students.map(s => s.id));
    const student = {id: maxId + 1, name, age};

    // ส่วนนี้จาก 5.1 middleware จัดการแล้ว
    // if (!name || !age) {
